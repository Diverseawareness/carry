...
| 3a | **Quick Game scorer must be a Carry user** — every QG tee group has a `canScore == true` player in slot 0; guests, pending invites, SMS invitees never become scorers | [scorer-rules.md](scorer-rules.md) §"Foundational premise" + [game-types.md](game-types.md) §Invariants | `canSave` validation in [QuickStartSheet.swift:116](../../Carry/Views/QuickStartSheet.swift:116) blocks Create when violated. 🚧 also worth: Swift test asserting `canSave` returns false when slot 0 has no profileId |
