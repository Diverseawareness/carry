...
struct GroupsListView: View {
    /// Format used by `QuickStartSheet.createQuickGame` to auto-generate
    /// a Quick Game's name from its tee time at creation. Used in
    /// `onTeeTimeChanged` to detect whether a Quick Game's `name` is
    /// still auto-generated (vs. manually renamed) when the date later
    /// changes — exact-match only, so user-typed names are preserved.
    private static let quickGameAutoNameFormatter: DateFormatter = {
        let f = DateFormatter()
    ...
    private func roundCoordinatorOverlay(group: SavedGroup) -> some View {
        ...
        RoundCoordinatorView(
            ...
            onTeeTimeChanged: { newDate in
                // Capture state needed for the auto-rename decision BEFORE
                // we mutate scheduledDate below.
                var quickGameAutoRename: (newName: String, groupId: UUID)? = nil
                if let idx = groups.firstIndex(where: { $0.id == group.id }) {
                    let oldDate = groups[idx].scheduledDate
                    let isQuickGame = groups[idx].isQuickGame
                    // Quick Games are auto-named "EEE, MMM d" at creation
                    // (QuickStartSheet.createQuickGame). If the user later
                    // changes the date, the title goes stale unless we
                    // refresh it. Only auto-rename when the current name
                    // still EXACTLY matches the auto-format applied to the
                    // OLD date — manual renames don't match and are
                    // preserved untouched. Both old and new dates must be
                    // non-nil to compute the comparison + new name.
                    let nameLooksAutoGenerated = oldDate.map {
                        groups[idx].name == Self.quickGameAutoNameFormatter.string(from: $0)
                    } ?? false
                    groups[idx].scheduledDate = newDate
                    if isQuickGame, nameLooksAutoGenerated, let newDate {
                        let newAutoName = Self.quickGameAutoNameFormatter.string(from: newDate)
                        groups[idx].name = newAutoName
                        quickGameAutoRename = (newName: newAutoName, groupId: groups[idx].id)
                    }
                }
                if authService.isAuthenticated {
                    Task {
                    ...
                    if let rename = quickGameAutoRename {
                        Task {
                            try? await GroupService().updateGroup(
                                groupId: rename.groupId,
                                update: SkinsGroupUpdate(name: rename.newName)
                            )
                        }
                    }
                }
